"""Contains the MAEGridArchive from CMA-MAE.

Adapted from
https://github.com/icaros-usc/cma_mae/blob/main/ribs/archives/_grid_archive.py
"""
import gin
import numpy as np
...
from src.archives.mae_archive_base import MAEArchiveBase

_EPSILON = 1e-6


@gin.configurable
class MAEGridArchive(MAEArchiveBase):
    ...
    def __init__(self,
        ...
        # Copied from src.archives.GridArchive
        self._record_history = record_history
        self._history = [] if self._record_history else None

    # Methods from GridArchive.
    best_elite = GridArchive.best_elite
    new_history_gen = GridArchive.new_history_gen
    history = GridArchive.history
    ...
    def add(self, solution, objective_value, behavior_values, metadata=None):
        ...
        # Save obj and BCs in the history, even if the solution was not inserted
        # into the archive.
    ...
    def get_index(self, behavior_values):
        ...
        """
        index = MAEGridArchive._get_index_numba(behavior_values,
                                                self._upper_bounds,
                                                self._lower_bounds,
