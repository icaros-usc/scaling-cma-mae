...
Adapted from Fontaine 2022:
https://github.com/icaros-usc/cma_mae/blob/main/ribs/emitters/_annealing_emitter.py
"""
import itertools
import logging
...
from src.emitters.opt.get_class import get_class
...
class AnnealingEmitter(EmitterBase):
    ...
    Args:
        ...
        seed (int): Value to seed the random number generator. Set to None to
            avoid a fixed seed.
        es (str): Which es to use. See src.emitters.opt.get_class.
    Raises:
    ...
    def __init__(self,
        ...
        opt_class = get_class(es)
        logger.info("Using ES class %s", opt_class)
        opt_seed = None if seed is None else self._rng.integers(10_000)
        self.opt = opt_class(sigma0, batch_size, self._solution_dim,
                             weight_rule, opt_seed, self.archive.dtype)
    ...
    @property
    def restarts(self):
        """int: Number of restarts so far in this emitter."""
    ...
    def _check_restart(self, num_parents):
        ...
        """
        if self._timeout is not None and self._timeout <= self._restart_timer:
            return True
    ...
    def tell(self,
        ...
        self.opt.tell(solutions[indices], self._num_parents, indices)
