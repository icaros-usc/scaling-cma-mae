...
from src.archives import GridArchive, MAEGridArchive
from src.emitters import (AnnealingEmitter, GaussianEmitter,
                          GradientImprovementEmitter, IsoLineEmitter,
                          PGAEmitter)
...
class Manager:  # pylint: disable = too-many-instance-attributes
    ...
    Algorithm args:
        ...
        result_archive_type: Archive class for the result archive. Intended
            for gin configuration.
    ...
    def __init__(
        ...
        # Setting up a virtual display to solve the open gl context error.
        # The error is encountered when the env need to render to a display
        ...
        setup_display()
        client.register_worker_callbacks(setup_display)
        ...
        if not reload:
            ...
            # pyribs result archive.
            if result_archive_type is None:
                result_archive = None
            else:
                result_archive = result_archive_type(seed=seed,
                                                     dtype=np.float32)
            logger.info("Result Archive: %s", result_archive)
            ...
            for emitter_class, n_emitters in emitter_types:
                emitter_seeds = self._rng.integers(np.iinfo(np.int32).max,
                                                   size=n_emitters,
                                                   endpoint=True)
                if str(emitter_class) == str(GradientImprovementEmitter):
                    # gin seems to interfere with the emitter class, so we have
                    # to compare the strings because emitter_class ==
                    # GradientImprovementEmitter will always be false.
                    emitters.extend([
                        emitter_class(
                            archive,
                            initial_solution,
            ...
            # pyribs optimizer.
            self._optimizer = Optimizer(archive, emitters, result_archive)
    ...
    def _msg_all(self, msg: str):
        ...
        with self._logdir.pfile("dashboard_status.txt").open("w") as file:
            file.write(msg)
    ...
    def _save_archive(self):
        ...
        df = self._optimizer.result_archive.as_pandas(include_solutions=True,
    ...
    def _save_archive_history(self):
        ...
        with self._logdir.pfile("archive_history.pkl").open("wb") as file:
            pkl.dump(self._optimizer.result_archive.history(), file)
    ...
    def _add_performance_metrics(self):
        ...
        df = self._optimizer.result_archive.as_pandas(include_solutions=False)
        objs = df.batch_objectives()
        stats = self._optimizer.result_archive.stats
    ...
    def _evaluate_solutions(self, with_gradients: bool):
        ...
        tell_kwargs = []
        for i, e in enumerate(self._optimizer.emitters):
            tell_kwargs.append(e_kwargs := {})
            if isinstance(e, GradientImprovementEmitter) and with_gradients:
                e_kwargs["grad_estimate"] = True
                e_kwargs["td3"] = self._td3

                # TODO: This is sketchy since it relies on each emitter only
                # outputting one solution in the ask-tell loop for DQD.
                if len(results) == len(self._optimizer.emitters):
                    e_kwargs["experience"] = results[i].experience
    ...
    def _calc_best_robustness(self):
        ...
        elite = self._optimizer.result_archive.best_elite()
    ...
    def _get_summary(self, generation) -> float:
        """Returns string summary of algorithm performance."""
        qd_score = self._metrics.get_single('Actual QD Score')['y'][-1]
        best = ("N/A" if generation <= 1 else
                f"{self._metrics.get_single('Best Performance')['y'][-1]:.3f}")
        coverage = self._metrics.get_single('Archive Coverage')['y'][-1]
    ...
    def execute(self):
        ...
        for generation in range(self._generations_completed + 1,
                                self._generations + 1):
            self._msg_all("-- "
                          f"Generation {generation}/{self._generations} | "
                          f"{self._get_summary(generation)}"
                          " --")
            self._metrics.start_itr()
            self._optimizer.archive.new_history_gen()
            self._optimizer.result_archive.new_history_gen()
        ...
        self._msg_all(f"-- Done! | {self._get_summary(generation)} --")
